...
final class Bootstrap
{
    ...
    private function usageHelp(): string
    {
        ...
        Available commands                                                              
          CreateConfig [bridge_ip]   Detect the Hue Bridge, create an user and store
                                       into config.json.
                                       - bridge_ip: Force Hue Bridge IP, useful when
                                         autodetection fails or when there are multiple
                                         Hue Bridges in the same network.
          DeleteResourceLinks <id>   Delete resource links by id
          DeleteScene <id>           Delete a scene by id
          DeleteUnusedMemorySensors  Delete memory sensors (boolean and integer flags)
                                       that are not used by any rules
